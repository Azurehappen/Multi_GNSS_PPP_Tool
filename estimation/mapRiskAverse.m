function [x_post,P_post,b,augcost,J_out,exitflag] = mapRiskAverse(y,H,P,R,J_l,x_prior)
...
diagRs = sqrt(diagR);
sH     = inv(diag(diagRs))*H;   % scale row i by {\sigma_i)^{-1}
...
new_J_L   = diag(Jpminus(1:3,1:3)) - ieqRHS; 
...
while cnt_it < 10
        ...
        switch exitflag
            ...
            case -9
                ...
                while exitflag == -9
                    ...
                    [b,~,exitflag,~] = intlinprog(cost_b,intcon,ieqLHS,ieqRHS,Aeq,Beq,lowerbound,upperbound,option);
                    if (exitflag <= 0)
                        disp('Solver still unsuccesdful')
                        exitflag
                    end
                end
            otherwise
    ...
    if (exitflag >= 1)
        ...
        C(2*cnt_it) = cost(xcurr,x_prior,P,b,H,y,R);
        ...
        ind = find(ieqRHS < max_neg_lhs);
        ieqRHS(ind) = 0.8 * max_neg_lhs(ind);    % loosen constraint
        x_post = xcurr;                    % try again
        flg_feasbl = false;
    end
    %b'
    xcurr = x_post;
    C(2*cnt_it+1) = cost(xcurr,x_prior,P,b,H,y,R);
    if and(abs(C(2*cnt_it+1)-C(2*cnt_it-1))<0.001,flg_feasbl)
        break
    end
    cnt_it = cnt_it + 1;
end % while cnt_it
